...
6. Prints a ready-to-paste C++ protocol table for the ESPHome component.

Usage:
    python3 tools/decode.py captures/
    python3 tools/decode.py captures/*.txt --emit-cpp
...
def emit_cpp(frames: Sequence[Frame], analysis: Dict) -> None:
    timing: Timing = analysis["timing"]
    width: int = analysis["reference_width"]
    attribution: Attribution = analysis["attribution"]
    checksums: List[ChecksumMatch] = analysis["checksums"]
    checksum_bits: Set[int] = analysis["checksum_bits"]

    print(banner("C++ PROTOCOL TABLE (paste into lg_portable_ac.h)"))
    print(
        f"""
// --- generated by tools/decode.py -------------------------------------------
static const uint8_t FRAME_BITS = {width};

// YAML defaults, in microseconds.
//   header_high: {timing.header_mark}us
//   header_low:  {timing.header_space}us
//   bit_high:    {timing.bit_mark}us
//   bit_one_low: {timing.one_space}us
//   bit_zero_low: {timing.zero_space}us"""
    )
    if timing.gap_space:
        print(f"//   chunk_gap_low: {timing.gap_space}us")
        if frames and frames[0].chunk_boundaries:
            print(f"//   chunk_bits: {frames[0].chunk_boundaries[0]}")
    ...
    for key in STATE_KEYS:
        ...
        if not field_bits:
            continue
        spans = runs(field_bits)
        # Several runs for one key almost always means one wider field whose
        # remaining bits happened to stay constant, so emit the enclosing span
        # and flag it rather than inventing several fields.
        start, end = spans[0] if len(spans) == 1 else enclosing_span(field_bits)
        suffix = "" if len(spans) == 1 else "  // TODO widened from several runs; verify"
        print(
            f"static const BitField FIELD_{key.upper()} = {{ /* shift */ {width - 1 - end}, "
            f"/* width */ {end - start + 1} }};{suffix}"
        )

    if checksums:
        best = checksums[0]
        print(
            f"static const ChecksumType CHECKSUM = CHECKSUM_{best.algorithm.upper()};  "
            f"// {best.checksum_nibbles * 4} bits, skip {best.skip_nibbles} nibble(s), "
            f"offset 0x{best.offset:X}"
        )
    else:
        print("// checksum: UNRESOLVED - see PROTOCOL.md")
    print("// ---------------------------------------------------------------------------")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--emit-cpp", action="store_true", help="print a C++ protocol table")
